...
3 9 v   -1
```
We generate a map with the cart positions updated with the respective track sections. We take
...
"  \\------/   "
```
We perform an iteration. It has no specific end condition, but there is a return statement in the
middle.
```q
    while[1b;
        ...
...
If we are in part 1, we return here. The return value is the coordinates converted into a string,
